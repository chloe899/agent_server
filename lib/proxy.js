...
function getRequestPostOnly(externalUrl, headers, dataStr){
...
function replaceProxyRedirectHost(req, res){
    ...
    if(/^http(s)?:\/\/?/i.test(url)){
        url = txtUtil.replaceProxyHost(req, url, true);
       ...
       log.debug("redirect url is %s",url);
    }
    ...
    async.waterfall([
            function(cb){
                if(method == "POST"){
                    ...
                    var shouldPipe = proxyHeaders["content-type"] == "application/x-amf";
                    if(shouldPipe){
                        log.debug("post headers is %s", proxyHeaders);
                        var sReq = getRequestPostOnly(url, proxyHeaders, data, function(err, pRes, body){
                            cb(err, pRes, body);
                        });
                        sReq.pipe(res);
                        cb("handled");
...
that.handleResponse = function(originReq, originRes, pRes, callback){
    ...
    async.waterfall([
        function(cb){
            ...
            log.debug("compress method is %s", encoding);
            if(isT){
                switch (encoding) {
                    ...
                    case 'gzip':
                        ...
                        break;
                    case 'deflate':
                        zlib.inflateRaw(body, function(err, buffer) {
                            cb(err, buffer);
                        });
                        break
                    default :
        ...
        if(isT){
            ...
            if(typeof(result) == "string"){
                result   = txtUtil.replaceProxyHost(req, result);
