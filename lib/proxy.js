    ...
    async.waterfall([
        function(cb){
            if(method == "POST"){
                ...
                console.log("is match %s",pathReg.test(pathInfo.pathname));
                //Content-disposition
                /*if(pathReg.test(pathInfo.pathname)){
                    var options = {url:url, headers:proxyHeaders};
